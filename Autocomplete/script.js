...
let inputBox = document.querySelector('#country');
...
inputBox.addEventListener('input', () => {
    ...
    if(inputValue != '') {
        for(let i = 0; i < countries.length; i++) {
            ...
            let countrySubstring = country.substring(0, inputLength);
            if(inputValue.toUpperCase() == countrySubstring.toUpperCase() && inputValue.toUpperCase() != country.toUpperCase()) {
                correctCountry.push(country);
            }
            
        }
        setCountries(correctCountry);
    } else {
        answer.textContent = '';
    }
});
...
function setCountries(correctCountry) {
    answer.textContent = '';
    for(let i = 0; i < correctCountry.length; i++) {
        let newBox = document.createElement('p');
        newBox.textContent = correctCountry[i];
        answer.appendChild(newBox);
    }
}